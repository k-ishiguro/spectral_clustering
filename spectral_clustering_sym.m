function Z = spectral_clustering_sym(X, K, fig_flag)
% Z = spectral_clustering_sym(X, K, fig_flag)
%
% implementation of the spectral clustering with 
% normalized symmetric graph laplacian. 
% This works, but not recommended. 
% Recommended to use spectral_clustering.m. 
%
% Reference: 
...
%% compute the symmetric normalized matrix L
D_safe = diag(D); % vectorized
D_coef = diag(EPS + 1.0 / sqrt(D_safe));

L = eye(N) - D_coef * X * D_coef;
...
if(fig_flag)
    ...
    ylabel('i')
    axis ij
    title('Symmetric Normalized Laplacian matrix L')
    colorbar
end

%% solve the eigen decomposition
[V, S] = eig(L);
...
% before K-means, normalize
v_norm = sum(Vk .* Vk, 2);
v_norm = sqrt(v_norm + EPS);
Vk = Vk ./ repmat(v_norm, [1 K]);
